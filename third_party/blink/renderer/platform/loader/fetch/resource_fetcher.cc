...
Resource* ResourceFetcher::RequestResource(FetchParameters& params,
  ...
  if (ResourceNeedsLoad(resource, params, policy)) {
    ...
    // :: Forensics :: here to instrument the resource during replay
    // the factory handles how to create the resource already
    // we update the resource during replay
    auto* resourceDataStore = ResourceDataStore::GetInstance();
    bool isReplayed = false;
    if (resourceDataStore->replaying()) {
        isReplayed = resourceDataStore->ReplayResponse(resource, task_runner_);
      if (isReplayed) return resource;
    }
    // Forensic end, we will fall back to real network request for not matching URLs.
    if (!isReplayed && !StartLoad(resource,
                   std::move(params.MutableResourceRequest().MutableBody()))) {

      if (resourceDataStore->replaying())
        DCHECK(nullptr) << "Attempting to fallback to regular replay.";
...
void ResourceFetcher::HandleLoaderFinish(Resource* resource,
  ...
  if (type == kDidFinishLoading) {
    // :: Forensics ::
    // we put recorder here to capture the responses. Log the data before it is
    // cleaned
    LoaderRecorder::DidFinishLoadingResource(resource);
    //
...
void ResourceFetcher::HandleLoaderError(Resource* resource,
  ...
  // :: Forensic ::
  // TODO (ian): handle finishAsError. We may not need to as during replay, we just
  // acknowledge that
...
bool ResourceFetcher::StartLoad(Resource* resource,
  ...
  // :: Forensics ::
  // during replay, we don't really start
  //
  loader->Start();
