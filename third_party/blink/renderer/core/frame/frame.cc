...
// :: Forensics ::
void Frame::SetReplayId(WTF::String replay_id) {
  replay_frame_id_ = replay_id;
}

WTF::String Frame::GetReplayId(){
  if (ForensicReplayEngine::GetInstance()->replay()) {
    return replay_frame_id_;
  } else {
    return IdentifiersFactory::FrameId(this);
  }
}
// Forensics
...
Frame::Frame(FrameClient* client,
  ...
  // :: Forensics ::
  SetReplayId(IdentifiersFactory::FrameId(this));
  init_cnt = 0;
  //
}
...
void Frame::Initialize() {
  ...
  // :: Forensics ::
  DOMModificationRecorder::FrameInitialized(this);
  init_cnt++;
  // Forensics
  ...
  if (owner_) {
    ...
    // :: Forensics ::
    if (IsLocalFrame()) {
      HTMLFrameOwnerElement *html_owner =
          DynamicTo<HTMLFrameOwnerElement, FrameOwner>(owner_);


      DOMModificationRecorder::DidInsertDOMFrame(html_owner);

      auto *engine = ForensicReplayEngine::GetInstance();
      if (engine->replay()) {
        VLOG(7) << "FrameOwner: " << html_owner->GetDocument().Url() << " " << init_cnt;
        DCHECK("https://a37ea822737ec24218520596987f207a.safeframe.googlesyndication.com/safeframe/1-0-38/html/container.html" != html_owner->GetDocument().Url());
        engine->RegisterFrameCreation(DynamicTo<LocalFrame>(this), html_owner);
      }
      // Forensics
    }
  }
