...
void LocalFrame::Navigate(FrameLoadRequest& request,
  ...
  if (request.ClientRedirectReason() != ClientNavigationReason::kNone) {
    ...
    // :: Forensics ::
    NavigationRecorder::FrameScheduledNavigation(this,
  ...
  if (client_redirect_reason != ClientNavigationReason::kNone) {
    ...
    // :: Forensics ::
    NavigationRecorder::FrameClearedScheduledNavigation(this);
    //
  }
}
...
void LocalFrame::SetDOMWindow(LocalDOMWindow* dom_window) {
  ...
  VLOG(7) << "Resetting DOM window:";
  if (this->DomWindow())
    this->DomWindow()->Reset();
  VLOG(7) << "Finished resetting DOm window";
  GetScriptController().ClearWindowProxy();
  VLOG(7) << "Clearing window proxy finsihed";
  dom_window_ = dom_window;
  VLOG(7) << "Set DOM_window_" << dom_window_;
}
...
void LocalFrame::DownloadURL(
  ...
  // :: Forensics ::
  NavigationRecorder::WillDownloadURL(this, params->url, params->suggested_name);
  //
  GetLocalFrameHostRemote().DownloadURL(std::move(params));
}
