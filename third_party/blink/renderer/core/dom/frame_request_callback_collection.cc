...
FrameRequestCallbackCollection::RegisterFrameCallback(FrameCallback* callback) {
  ...
  // :: Forensics, if replaying, do not insert. We will invoke callback ourselves.
  if(ForensicReplayEngine::GetInstance()->replay()) {
    auto* engine = ForensicReplayEngine::GetInstance();
    engine->RegisterAnimationFrameEvent(context_, callback);
    return id;
  }
