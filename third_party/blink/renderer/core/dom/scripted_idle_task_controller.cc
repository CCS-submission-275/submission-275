...
ScriptedIdleTaskController::RegisterCallback(
  ...
  if (ForensicReplayEngine::GetInstance()->replay()) {
    ...
    engine->RegisterIdleCallbackEvent(GetExecutionContext(), id);
    return id;
  }
  // Forensics
...
void ScriptedIdleTaskController::RunCallback(
  ...
  // :: Forensics ::
  TimerRecorder::RecordIdleCallbackEventBegin(
          GetExecutionContext(), id, callback_type, allotted_time);
  // Forensics
  idle_task->invoke(
      MakeGarbageCollected<IdleDeadline>(deadline, callback_type));
  // :: Forensics ::
  TimerRecorder::RecordIdleCallbackEventEnd(
          GetExecutionContext(), id, callback_type);
  // Forensics
