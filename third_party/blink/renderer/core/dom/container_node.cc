...
void ContainerNode::InsertNodeVector(
  ...
  // :: Forensics ::
  // DOMModificationRecorder::WillInsertDOMNode(this);
  // Forensics
  {
    ...
    for (const auto& target_node : targets) {
      ...
      // :: Forensics ::
      // Note (ian): It seems like this insertion is made by API calls as said
      // in DidInsertNodeVector
      DOMModificationRecorder::DidInsertDOMNode(&child,
                                                ChildrenChangeSource::kAPI);
      // if (ForensicReplayEngine::GetInstance()->replay()) {
      //   ForensicReplayEngine::GetInstance()->VerifyDOMEvent(&child,
      //   ForensicDOMEventType::kInsertNode);
      // }
      // Forensics
...
void ContainerNode::NotifyNodeInserted(Node& root,
  ...
  // :: Forensics ::
  DOMModificationRecorder::DidInsertDOMNode(&root, source);
  // if (ForensicReplayEngine::GetInstance()->replay()) {
  //   ForensicReplayEngine::GetInstance()->VerifyDOMEvent(&root,
  //   ForensicDOMEventType::kInsertNode);
  // }
  // Forensics
  NodeVector post_insertion_notification_targets;
...
static void DispatchChildRemovalEvents(Node& child) {
  if (child.IsInShadowTree()) {
    ...
    // :: Forensics ::
    DOMModificationRecorder::WillRemoveDOMNode(&child);
    // if (ForensicReplayEngine::GetInstance()->replay()) {
    //   ForensicReplayEngine::GetInstance()->VerifyDOMEvent(
    //       &child, ForensicDOMEventType::kRemoveNode);
    // }
    // Forensics
