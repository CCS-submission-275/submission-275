...
void Node::RemoveAllEventListenersRecursively() {
  VLOG(7) << "Being removing all nodes";
  ScriptForbiddenScope forbid_script_during_raw_iteration;
  for (Node& node : NodeTraversal::StartsAt(*this)) {
    // VLOG(7) << "Removing all event listeners";
    node.RemoveAllEventListeners();
    // VLOG(7) << "Finish Removing all event listeners";
    if (ShadowRoot* root = node.GetShadowRoot())
      root->RemoveAllEventListenersRecursively();
  }
  VLOG(7) << "Returning all event listenres!";
}
  ...
  if (event_type == event_type_names::kKeydown ||
    ...
    if (auto* keyboard_event = DynamicTo<KeyboardEvent>(&event)) {
      if (LocalFrame* frame = GetDocument().GetFrame()) {
        // :: Forensics ::
        EventRecorder::RecordDispatchEventStart(&event, true);
        //
        frame->GetEventHandler().DefaultKeyboardEventHandler(keyboard_event);
        // :: Forensics ::
        EventRecorder::RecordDispatchEventEnd(&event, true);
        //
      }
    }
    ...
    if (Page* page = GetDocument().GetPage()) {
      // :: Forensics ::
      // EventRecorder::RecordDispatchEventStart(event.target(), &event);
      //
      page->GetContextMenuController().HandleContextMenuEvent(
          To<MouseEvent>(&event));
      // :: Forensics ::
      // EventRecorder::RecordDispatchEventEnd(event.target(), &event);
      //
    }
