...
void ClassicPendingScript::NotifyFinished(Resource* resource) {
  ...
  //bool decode_error = GetResource()->GetStatus() == ResourceStatus::kDecodeError;
  //bool load_error = GetResource()->GetStatus() == ResourceStatus::kLoadError;
  //DCHECK(!error_occurred) << "loader error: " << load_error << "decode error: " << decode_error
  //<< "integ failure_" << integrity_failure_  << " " << UrlForTracing();
...
ClassicScript* ClassicPendingScript::GetSource(const KURL& document_url) const {
  ...
  if (!is_external_) {
    ...
    //VLOG(7) << "source code: " << source_code.Source().ToString();
  ...
  VLOG(7) << "is streamer ready: " << streamer_ready;
...
void ClassicPendingScript::AdvanceReadyState(ReadyState new_ready_state) {
  ...
  if (ForensicReplayEngine::GetInstance()->replay()) {
    ...
    engine->RegisterClassicPendingScript(this);
  }
  // Forensics
