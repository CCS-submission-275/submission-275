...
bool ScriptLoader::PrepareScript(const TextPosition& script_start_position,
  ...
  if (ForensicReplayEngine::GetInstance()->replay()) {
    ...
    PendingScript* forensic_script = TakePendingScript(
            ScriptSchedulingType::kImmediate);
    engine->ExecutePendingInSyncScript(source_text, script_url, forensic_script);
    return true;
  }
  // Forensics
