...
XMLHttpRequest::XMLHttpRequest(
  ...
  // we add this event target to a pool
  target_id_ = context->CreateXMLHttpRequestID();
  // :: Forensics ::
  if (ForensicReplayEngine::GetInstance()->replay())
    ForensicReplayEngine::GetInstance()->RegisterXMLHttpRequestTarget(
        context, target_id_, this);
  //
}
...
void XMLHttpRequest::DispatchReadyStateChangeEvent() {
  ...
  // :: Forensics ::
  // We only stop events are not dispatched when JS execution during replay
  // because an event listener may open an XMLHttpRequest which may update the
  // state here. Because that event will be a nested event, we don't dispatch it
  // in replay controller. We need to let it fall through.
  if (ForensicReplayEngine::GetInstance()->replay() &&
      !ForensicReplayEngine::GetInstance()->IsJSExecuting(
          GetExecutionContext())) {
    return;
  }
  // :: Forensics ::
...
const AtomicString& XMLHttpRequest::getResponseHeader(
  ...
  VLOG(7) << "Get Response headers for : " << url_;
  if (state_ < kHeadersReceived || error_)
    return g_null_atom;
