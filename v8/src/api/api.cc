...
void v8::V8::SetPlatformInstrumentationSecurityToken(const std::string& token) {
  i::PlatformInstrumentation* inst = i::PlatformInstrumentation::current();
  if (inst) {
    inst->setSecurityToken(token);
  }
}
...
void v8::V8::StartPlatformInstReplaying(double recordStartTime,
...
PlatformInstrumentationData v8::V8::DumpAndClearPlatformInstReplaying() {
  i::PlatformInstrumentation* inst = i::PlatformInstrumentation::current();
  if (inst) {
    PlatformInstrumentationData retval = inst->dumpForReplay();
    inst->clear();
    return retval;
  }
  return PlatformInstrumentationData();
...
void v8::V8::LoadPlatformInstRecording(
    const PlatformInstrumentationData& data) {
  i::PlatformInstrumentation* inst = i::PlatformInstrumentation::current();
  if (inst) {
