...
// :: Foresncis ::
TNode<Smi> CodeStubAssembler::InstrumentMathRandom(
    TNode<NativeContext> native_context) {
  // Cache exhausted, populate the cache. Return value is the new index.
  const TNode<ExternalReference> instrument_math_random =
      ExternalConstant(ExternalReference::instrument_math_random());
  MachineType type_tagged = MachineType::AnyTagged();

  return CAST(CallCFunction(instrument_math_random, type_tagged,
                std::make_pair(type_tagged, native_context)));
}
//
