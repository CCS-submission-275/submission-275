...
/**
 * instrumentation data :: Forensics ::
 */
class V8_EXPORT PlatformInstrumentationData {
 public:
  virtual ~PlatformInstrumentationData() {}
  std::unordered_map<std::string, std::vector<double>> current_timestamps_;
  std::unordered_map<std::string, std::vector<double>> math_random_values_;

  size_t size() { return size(current_timestamps_) + size(math_random_values_); }

  size_t size(
      std::unordered_map<std::string, std::vector<double>>& src) {
    size_t retval = 0;
    std::unordered_map<std::string, std::vector<double>>::const_iterator end =
        src.end();
    for (std::unordered_map<std::string, std::vector<double>>::const_iterator
             it = src.begin();
         it != end; ++it) {
  ...
  static size_t size(const std::vector<double>& src) {
    ...
    for (std::vector<double>::const_iterator it = src.begin(); it != end;
         it++) {
      retval += sizeof(*it);
    }
    return retval;
...
class V8_EXPORT V8 {
 public:
  ...
  static void SetPlatformInstrumentationSecurityToken(const std::string& token);
  ...
  static PlatformInstrumentationData DumpAndClearPlatformInstReplaying();
